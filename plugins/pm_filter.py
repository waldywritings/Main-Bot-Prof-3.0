...
async def pm_spoll_choker(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("I couldn't find any movie in that name.", quote=False)
    ...
    if not movielist:
        k = await msg.reply("I couldn't find anything related to that. Check your spelling", quote=False)
        await asyncio.sleep(8)
        await k.delete()
        return
